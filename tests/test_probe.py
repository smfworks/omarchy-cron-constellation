...
def test_normalize_cost_never_invents_usd():
    tokens, usd, estimated = api.normalize_cost({
        "prompt_tokens": 100,
...
def test_normalize_cost_uses_recorded_actual_usd_only():
    ...
    assert estimated == pytest.approx(0.04)


def test_normalize_cost_skips_unknown_and_included():
    _, usd_unknown, est_unknown = api.normalize_cost({
        "estimated_cost_usd": 1.5,
        "cost_status": "unknown",
    })
    _, usd_included, est_included = api.normalize_cost({
        "estimated_cost_usd": 0.0,
        "cost_status": "included",
    })
    assert usd_unknown is None
    assert est_unknown is None
    assert usd_included is None
    assert est_included is None


def test_normalize_cost_does_not_coerce_missing_to_zero():
    tokens, usd, estimated = api.normalize_cost({"job_id": "x"})
...
def test_session_actual_cost_is_used_when_present(tmp_path: Path):
    ...
    assert runs[0]["estimatedUsd"] == pytest.approx(0.01)
...
def test_normalize_cost_cache_only_counts():
    tokens, usd, estimated = api.normalize_cost({"cache_read_tokens": 40, "cache_write_tokens": 2})
    assert tokens == 42
    assert usd is None
    assert estimated is None
...
def test_normalize_cost_keeps_estimated_off_the_usd_field():
    tokens, usd, estimated = api.normalize_cost({
        "total_tokens": 10,
    ...
    assert tokens == 10
    assert usd is None
    assert estimated == pytest.approx(1.25)


def test_empty_home_is_demo(tmp_path: Path):
...
def test_build_snapshot_unread_jobs_is_error_not_quiet(tmp_path: Path):
    ...
    assert snap["read_status"] == "unread"
    assert snap["error"]
    assert snap["runs"] == []
