"""Cron Constellation — overnight Hermes cron runs for the Omarchy bar.

Same storage spirit as smf-cron-night ``plugin_api.py``. Reads profile-local
cron state under each Hermes home (default ``~/.hermes`` and
...
Never invents a dollar amount. Tokens and USD are nullable. ``python3 probe.py``
prints one JSON snapshot for the bar widget. Pass ``--tz`` (IANA) so the
overnight window matches the Omarchy shell, not a UTC gateway. Omitted/invalid
``tz`` uses the process local zone.
"""
...
def normalize_cost(payload: Any) -> Tuple[Optional[int], Optional[float], Optional[float]]:
    """Return ``(tokens, usd, estimated_usd)``. Never priced from tokens."""
    ...
    if cost_status not in {"unknown", "n/a", "na"}:
        for key in (
            ...
            if usd is not None:
                break
        estimated = _finite_number(payload.get("estimated_cost_usd"))
...
def hermes_home_path(environ: Optional[dict] = None, root: Optional[Path] = None) -> Path:
    environ = environ or os.environ
    if root is not None:
        return Path(root)
...
def discover_hermes_homes(
    ...
    env = environ.get("HERMES_HOME")
    if env:
        add(Path(env))
    default = Path(root) if root is not None else user_home(environ) / ".hermes"
    add(default)
...
def _merge_run(target: Dict[str, Any], extra: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if usd_a is None and usd_b is not None:
        target["usd"] = usd_b
    est_a, est_b = target.get("estimatedUsd"), extra.get("estimatedUsd")
    if est_a is None and est_b is not None:
        target["estimatedUsd"] = est_b
    return target
...
def normalize_run(raw: Dict[str, Any], *, default_tz: tzinfo) -> Dict[str, Any]:
    ...
    if status is None and raw.get("error"):
        status = "failed"
    run["status"] = status
    tokens, usd, estimated = normalize_cost(raw)
    run["tokens"] = tokens
    run["usd"] = usd
    run["estimatedUsd"] = estimated
...
def night_payload(
    *,
    ...
    default_home = hermes_home_path(environ=environ, root=root)
    homes = discover_hermes_homes(default_home if root is not None else None, environ=environ)
    if root is not None and not homes:
        homes = discover_hermes_homes(root, environ=environ)
...
def demo_snapshot(
    ...
    zone = resolve_tz(tz_name)
    as_of = now.astimezone(zone) if now is not None else datetime.now(zone)
    start, end = overnight_window(now=as_of, tz=zone)
    tz_label = getattr(zone, "key", None) or str(zone)
    return {
        "present": False,
        "demo": True,
...
def error_snapshot(
    ...
    out = demo_snapshot(home, tz_name, now)
    out["demo"] = False
    out["ok"] = False
    out["error"] = message or "probe failed"
    out["read_status"] = "unread"
    out["errors"] = errors or [{"kind": "probe", "path": None, "error": out["error"]}]
    return out


def wrap_payload(payload: Dict[str, Any], home: Path, as_of: datetime) -> Dict[str, Any]:
    errors = payload.get("errors") or []
    ok = payload.get("ok") is True
    homes = payload.get("homes") or []
    summary = payload.get("summary") or empty_totals()
    out = dict(payload)
    out["present"] = True
    out["demo"] = False
    out["stale"] = False
    out["busy"] = int(summary.get("running") or 0) > 0
    if ok:
        out["error"] = ""
    elif errors:
        out["error"] = str(errors[0].get("error") or "unread cron storage")
    else:
        out["error"] = "unread cron storage"
    out["home"] = str(home)
    out["profileCount"] = len(homes)
    out["probedAt"] = as_of.timestamp()
    out["plugin"] = PLUGIN
    return out
...
def build_snapshot(
    ...
    environ = environ or os.environ
    zone = resolve_tz(tz_name or environ.get("TZ"))
    as_of = now.astimezone(zone) if now is not None else datetime.now(zone)
    home = hermes_home_path(environ=environ, root=root)
    try:
        payload = night_payload(
            tz_name=tz_name or environ.get("TZ"),
            root=root,
            now=as_of,
            environ=environ,
        )
    except Exception as exc:
        return error_snapshot(str(exc), home=str(home), tz_name=tz_name, now=as_of)

    discovered = payload.get("homes") or []
    if not discovered:
        return demo_snapshot(str(home), tz_name=tz_name, now=as_of)
    return wrap_payload(payload, home, as_of)


def main() -> None:
    parser = argparse.ArgumentParser(description="Cron Constellation Hermes overnight probe")
    parser.add_argument("--tz", default="", help="IANA timezone from the Omarchy shell")
    parser.add_argument("--now", default="", help="ISO timestamp override (tests)")
    args = parser.parse_args()
    tz = args.tz.strip() or os.environ.get("TZ") or None
    now = parse_datetime(args.now, default_tz=resolve_tz(tz)) if args.now.strip() else None
    print(json.dumps(build_snapshot(now=now, tz_name=tz), separators=(",", ":"), default=str))
